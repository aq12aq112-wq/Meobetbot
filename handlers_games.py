...
# نگهداری وضعیت بازی‌های فعال کاربران
active_pop_games = {}
active_dice_games = {} # {user_id: {"bet": amount, "choice": "even/odd", "rolls": []}}
...
async def start_pop_game(message: Message, state: FSMContext):
    ...
    # کسر مبلغ شرط از کاربر
    await update_balance(user_id, -amount)

    # ضرایب جدید پوپ به تفکیک مراحل (ایندکس 0 تا 4)
    multipliers = [1.2, 1.4, 1.6, 2.0, 2.5]

    stages = []
    for _ in range(5):
        row = [0, 0, 0, 1] # 3 تا امن، 1 پوچ
        random.shuffle(row)
        stages.append(row)
...
async def process_pop_click(callback: CallbackQuery):
    ...
    if is_poop:
        ...
        if game["current_stage"] >= 5:
            final_prize = game["bet"] * game["multipliers"][4] # ضریب مرحله پنجم (2.5)
...
async def handle_dice_throw(message: Message):
    ...
    if not game:
        return # اگر بازی تاس فعالی نداشت، کاری به کارش نداریم

    # بررسی اینکه نوع تاس ارسالی، تاس معمولی باشد (عدد 1 تا 6)
    ...
    if rolls_count < 3:
        ...
        # ۳ تاس کامل شد؛ محاسبه جمع و نتیجه
        total_sum = sum(game["rolls"])
        is_even = total_sum % 2 == 0
        user_choice = game["choice"] # "even" یا "odd"

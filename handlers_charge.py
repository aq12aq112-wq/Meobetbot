...
# ---------- برداشت وجه (بدون محدودیت کارت و دارای دکمه ادمین) ----------
...
async def cb_withdraw_start(callback: CallbackQuery, state: FSMContext):
    ...
    text = (
        ...
        "لطفاً شماره کارت میویی (یا مشخصات حساب) خود را وارد کنید:\n"
...
async def process_withdraw_amount(message: Message, state: FSMContext):
    ...
    # ثبت تراکنش برداشت برای مدیریت با دکمه ادمین
...
# مدیریت تأیید/رد برداشت توسط ادمین
...
async def admin_reject_withdraw(callback: CallbackQuery):
    ...
    await update_transaction_status(tx_id, "rejected")
    # برگشت دادن موجودی به کاربر چون درخواستش رد شد
    await update_balance(tx["user_id"], tx["amount"])
